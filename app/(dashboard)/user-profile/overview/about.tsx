        ...
        {driver ? (
          <>
            <div className="text-sm text-default-600 mb-3">
              Driver profile information for {driver.firstName || driver.lastName ? `${driver.firstName || ""} ${driver.lastName || ""}`.trim() : driver.email}.
              This driver has completed {driver.stats?.completedRides || 0} rides and earned {driver.stats?.totalEarnings || 0} {driver.currency || "EUR"} in total.
            </div>
            <div className="text-sm text-default-600">
              Account created on {format(new Date(driver.createdAt), "dd MMM yyyy")} and last updated on {format(new Date(driver.updatedAt), "dd MMM yyyy")}.
            </div>
          </>
        ) : (
          <>
        ...
        <div className='mt-6 flex flex-wrap items-center gap-6 2xl:gap-16'>
          {
            driver
              ? [
                  {
                    title: "Email",
                    position: driver.email,
                    icon: Mail2
                  },
                  {
                    title: "Phone",
                    position: driver.phone || "N/A",
                    icon: Phone
                  }
                ].map((item, index) => (
              <div
